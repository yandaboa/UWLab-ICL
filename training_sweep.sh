...
TRAIN_PM=false
DATASET_PATH_FORMAT='episodes/20260303_034130/episodes_0000{x}_trim.pt'
DEFAULT_BASE_CONFIG_PATH="/gscratch/weirdlab/yanda/lti/UWLab-yanda/source/uwlab_rl/uwlab_rl/rsl_rl/configs/copying.yaml"
            ...
            for LEARNING_RATE in "${LEARNING_RATES[@]}"; do
              for WEIGHT_DECAY in "${WEIGHT_DECAYS[@]}"; do
                for INCLUDE_CURRENT_TRAJECTORY in "${INCLUDE_CURRENT_TRAJECTORY_VALUES[@]}"; do
                  RUN_NAME="tracking_sweep_${DATASET_GROUP_INDEX}_seed${SEED}_hd${HIDDEN_DIM}_L${NUM_LAYER}_H${NUM_HEAD}_do${DROPOUT}_lr${LEARNING_RATE}_wd${WEIGHT_DECAY}_ict${INCLUDE_CURRENT_TRAJECTORY}"
