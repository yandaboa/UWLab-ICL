...
class ZarrDatasetFileHandler(DatasetFileHandlerBase):
    ...
    def _convert_and_save_episode(self, episode: EpisodeData):
        ...
        episode_data = {
            ...
            # Default to all-expert so datasets collected without the expert-mask recorder
            # still produce a valid loss mask downstream in diffusion_policy.
            "expert_mask": episode_dict.get("expert_mask", torch.ones((num_frames, 1))).cpu().numpy(),
        }
        if "expert_actions" in episode_dict:
